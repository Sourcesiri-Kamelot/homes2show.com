...
const BrokerAuth = ({ onAuthSuccess, onAuthFailure, userType = 'broker' }) => {
  ...
  const [stateCommissionInfo, setStateCommissionInfo] = useState(null);
  ...
  const handleInputChange = (e) => {
    ...
    // Update state commission info when state changes
    if (name === 'state' && value) {
      const commissionInfo = getRealEstateCommission(value);
      setStateCommissionInfo(commissionInfo);
    }
    ...
    try {
      ...
      if (result.verified) {
        ...
        localStorage.setItem('brokerAuth', JSON.stringify({
          ...
          stateCommission: stateCommissionInfo
        }));
    ...
    if (storedAuth) {
      try {
        ...
        if (authData.verified && new Date(authData.expiresAt) > new Date()) {
          ...
          if (authData.stateCommission) {
            setStateCommissionInfo(authData.stateCommission);
          }
  ...
  return (
    <div className="max-w-md mx-auto bg-white shadow-lg rounded-lg p-6">
      ...
      <div className="mt-4 text-xs text-gray-500">
        ...
        {stateCommissionInfo && (
          <p className="mt-2">
            Having issues? Contact {stateCommissionInfo.name} at {stateCommissionInfo.phone}
          </p>
        )}
      </div>
    </div>
